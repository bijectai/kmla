# SARA semantics: recommended entries for `human/DECISIONS.md`

Author: Fable (design authority), 2026-09-21, at Dev's request relayed in chat
(recorded as `Q-004`, answered in `A-004`).
Status: **accepted by Dev in chat on 2026-09-21 ("good to go, stay faithful
to what SARA authors intended").** In force once Dev copies it into
`human/DECISIONS.md`; until then it binds nothing. Every entry has an identifier so Dev can accept, edit, or
strike it individually. Entries that change a contract are marked
**[CONTRACT]** and mirrored as PROPOSED items in `docs/DECISION_LOG.md`.

Evidence basis. Every claim about interpreter behaviour was checked on the
pinned image already present on this machine (`kmla-swipl:7.2.3`, Debian 9,
SWI-Prolog 7.2.3 Multi-threaded 64 bits, `linux/amd64`, run under emulation on
this arm64 host). The probe programs and raw outputs are in
`docs/consult/evidence/`. Two facts about that image matter for Astra's
harness: it ignores command-line arguments under emulation (`swipl -g …`,
`-q`, `--dump-runtime-variables` all fall through to the banner), so every
probe was driven through stdin; and it has no `TZ` set, which is a semantic
input (D8). Nothing under `human/` was written.

Reading order. Section 0 lists the findings that change contracts. Section 1
gives the translation recipe every later entry assumes. Sections 2 to 9 follow
the `DECISIONS.md` schema. Section 10 is the hazard inventory cross-index.
Section 11 lists what Dev must verify before installing.

---

## 0. Findings that change contracts or block work
...
`\+ G` is `(solutionsOf G).isEmpty`; `!` per section 6. Multiplicity is kept
...
are excluded by `Valid` (V-rules, section 4.5). The harness must treat an
...
**G9 (fidelity rule, from Dev 2026-09-21).** Reproduce the authors' results
...
never uses. If Dev keeps the plan's cents, the equivalent entry is: all money
...
that. Proposed as P-MONEY in the decision log.
...
argument. The 21 sites of the form `round(C + (X − K) · R)` (section 1
...
* H4.4 (the two unterminated files). `s3306_c_2_neg.pl` and
  ...
  a documented reader exception (F2). Alternative if Dev rejects the repair:
  both cases are reported as "no executable test" and excluded from Week 1
...
on the taxable year; this is a PROPOSED change to the invariant form in
PROTOCOL B004 (P-VALID-YEAR). All of the following must hold:
...
* V8 (E5, head-of-household cycle). `¬ hohCycle h t y` for every person `t`,
  ...
  would exclude every §2(b)(1)(B) input and is not recommended.
...
`Valid` is stated in `Interface/`, may import `Oracle/` for V7/V8, and is the
premise of every invariant and of PROVED-EQUIV. The original 376 cases are
compared under `ValidStip h y` = V2–V9 (stipulations and the two `purpose_`
wildcards allowed) for Week 1 parity only.
...
accuracy (Phase 3.5, 4.3) is undefined for them under a single target. Proposed resolution:
...
* H6.4 (models and stipulations). Model outputs are graded on
  ...
  on all 376 through H4.1. Alternative A, if Dev wants every original to count
  for models: paragraph targets take an explicit environment record of the
  ...
  calling convention the statute text never mentions. Recommendation:
...
**R5 `s152_a_1 → s152_c → s152_c_1 → s152_c_1_E → s7703 → s7703_b → s7703_b_1
→ s152_a_1`.** Each round trip moves from a taxpayer to a child living with
them (`s7703_b_1` requires `s152_a_1(Dependent,Taxp)`), so the recursion
follows the child relation and terminates under V4. Fuel: `persons.length +
...
**R8 `s3306_c_10_A_ii → s7703 → s7703_b_1 → s152_a_1 → … → s7703`.** Covered
by R5's fuel.

**R9 (what fuel exhaustion means).** Exhaustion returns the empty solution
list. DECISIONS asserts that on `Valid` inputs no fuel is ever exhausted; the
miner may propose that as an invariant candidate per group, and a
counterexample would be a design flaw (a missing V-rule), not a grading
outcome.
...
## 10. Source hazard inventory

The tables in sections 2, 3, 5, 6, 7 are the inventory. Counts by file
...
## 11. What Dev must verify before installing

1. That the probes in `docs/consult/evidence/` reproduce on Dev's machine
   with the same image id (`de470fb4c535`) and that `TZ=America/New_York`
   yields 376/376 (`evidence/sweep_directives_ny.tsv`); the TZ pin is the
   single most consequential decision here.
2. That the money unit choice (M1) is the one Dev wants; every M-rule is
   written for dollars and the cents variant is spelled out.
3. That the observation is the solution *set* (H6.2) and not the ordered
   multiset; the oracle is written to reproduce order either way, the meter
   compares sets.
4. That `Valid` may be year-indexed (V8) and may import `Oracle/` (V7, V8).
5. That the stipulation-independent subset (H6.4) is acceptable for the
   per-case accuracy metric, and its size once the harness computes it.
6. That the two unterminated case files are to be read with the terminator
   restored (H4.4).
7. That F4 (unreachable "10 days" arms) is acceptable as a reported coverage
   gap rather than a reason to redesign the generator.
8. The assumption running through every table: the pinned interpreter's
